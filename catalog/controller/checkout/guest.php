...
class ControllerCheckoutGuest extends Controller {
	...
	public function save() {
		...
		if (!$json) {
			if ((utf8_strlen(trim($this->request->post['firstname'])) < 1) || !preg_match ('/^[a-zA-Z ]{2,30}$/', $this->request->post['firstname'])) {
				$json['error']['firstname'] = $this->language->get('error_firstname');
			}

			if ((utf8_strlen(trim($this->request->post['lastname'])) < 1) || !preg_match ('/^[a-zA-Z ]{1,30}$/', $this->request->post['lastname'])) {
			...
			if ((utf8_strlen($this->request->post['telephone']) < 3) || !preg_match ('/^[\d|\+|\(]+[\)|\d|\s|\W|-]*[\d]$/', $this->request->post['telephone'])) {
