...
class ModelCatalogWkallauctions extends Model {
	...
	public function getAuctions($filterValues){
		...
		if(!empty($filterValues['category'])){
		  $url .= "LEFT JOIN ".DB_PREFIX."product_to_category p2c ON p2c.product_id=a.product_id WHERE p2c.category_id ='".$filterValues['category']."' AND a.isauction=1 AND a.start_date <= '".$time."' AND a.end_date >= '".$time."' ORDER BY a.end_date ASC LIMIT ".$filterValues['start'].",".$filterValues['limit'];
		}else{
		  $url .="WHERE a.isauction=1 AND a.start_date <= '".$time."' AND a.end_date >= '".$time."' ORDER BY a.end_date ASC LIMIT ".$filterValues['start'].",".$filterValues['limit'];
		}
