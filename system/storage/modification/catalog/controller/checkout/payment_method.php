...
// Update Zipcode Starts
			...
			$zipcode_payment_methods = true;
			if(!empty($this->session->data['shipping_address']['postcode'])) {
				$cart_products = $this->cart->getProducts();
				foreach($cart_products as $cart_product) {
					$total_product_zipcodes = $this->model_catalog_product->getTotalProductZipcodes($cart_product['product_id']);
					$zipcode_info = $this->model_catalog_product->getProductInfoByZipCode($cart_product['product_id'], $this->session->data['shipping_address']['postcode']);
					if(empty($zipcode_info) && $total_product_zipcodes) {
						$zipcode_payment_methods = false;
					}
				}
			}

			if(!$zipcode_payment_methods) {
				if(isset($method_data['cod'])) {
